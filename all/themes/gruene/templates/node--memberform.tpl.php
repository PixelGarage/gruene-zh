...
<div class="middle">
	...
	<div class="title-container <?php print $type; ?>">
		...
		<?php if (isset($node->field_memberform_image) && !empty($node->field_memberform_image['und'][0]['uri'])): ?>
			<div class="title-image" style="background-image:url(<?php print file_create_url($node->field_memberform_image['und'][0]['uri']); ?>);"></div>
		<?php endif; ?>
		...
		<div class="form-container party-form">
			<?php
				$emailaddress = $node->field_memberform_email['und'][0]['value'];
				$name = "Test";
				webform_format_email_address($emailaddress, $name, 426);
				$webformnode = node_load(426);
				// Start Anpassungen Formular
				// Überschreibt den standard Email Empfänger mit dem in der Seite eingegebenen Email Feld
				if (!empty($node->field_memberform_email['und'][0]['value'])) {
					$webformnode->webform['emails'][1]['email'] = $node->field_memberform_email['und'][0]['value'];
				}
				if (isset($node->field_memberform_successmessage['und'][0]['value'])) {
					$webformnode->webform['confirmation'] =  $node->field_memberform_successmessage['und'][0]['value'];
				}
		...
		<div class="title-body">
			<?php if (isset($node->field_memberform_text)) print $node->field_memberform_text['und'][0]['value']; ?>
		</div>
